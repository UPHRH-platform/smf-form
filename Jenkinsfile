    ...
    try {
        ...
        ansiColor('xterm') {
            stage('Checkout') {
                ...
                env.commit_id = sh(script: "echo " + "thor" + "_" + commit_hash + "_" + env.BUILD_NUMBER, returnStdout: true).trim()
                echo "${env.commit_id}"
                }
        }
           stage('docker-pre-build') {
             sh '''
              ...
              docker run --name $docker_pre_build $docker_pre_build:latest && docker cp $docker_pre_build:/opt/target/form-service-0.0.1-SNAPSHOT.jar .
